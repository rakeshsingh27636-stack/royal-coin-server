...
// 👑 SECRET ADMIN PANEL ROUTE (Updated with Support Tickets)
...
let pendingSupport = {}; // ✅ SUPPORT TICKETS STORAGE
let rooms = {};
...
io.on('connection', (socket) => {
    ...
    socket.on('registerAdmin', async () => {
        ...
        socket.emit('adminViewSupport', pendingSupport); // Load tickets for admin
    ...
    socket.on('submitSupportTicket', async (data) => {
        ...
        if(u) {
            ...
            socket.emit('supportTicketResult', { status: 'success', message: 'Ticket submitted! Admin will check it shortly.' });
        }
    });
    ...
    socket.on('adminResolveTicket', (data) => {
        ...
        if(pendingSupport[data.ticketId]) {
            ...
            io.to(userSocket).emit('supportTicketResult', { status: 'resolved', message: 'Admin has resolved your support ticket.' });
